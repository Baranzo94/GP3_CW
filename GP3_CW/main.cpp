...
char* balloonTxtres[] = {"Images\\Ship.png","Images\\ShipGreen.png","Images\\ShipRed.png","Images\\explosion.png"};
		...
		case WM_LBUTTONDOWN:
			{
				...
				while (iter != aBalloon.end() && !gHit)
				{
					if ( (*iter)->insideRect((*iter)->getBoundingRect(),mouseXY))
					{
						...
						gExplodeSound.playSound(L"Sounds\\Laser.wav",false);
						gExplodeSound.playSound(L"Sounds\\explosion.wav",false);
						iter = aBalloon.erase(iter);
						gBalloonsBurst++;
						sprintf_s( gBalloonsBurstStr, 50, "Targets Destroyed : %d", gBalloonsBurst);
					}
					else
					{
						++iter;
...
bool initWindow( HINSTANCE hInstance )
{
	...
	wcex.lpszClassName = "Game"; // the class name being created
	wcex.hIconSm = LoadIcon(hInstance,"planet.ico"); // the handle to the small icon

	RegisterClassEx(&wcex);
	// Create the window
	wndHandle = CreateWindow("Game",			// the window class to use
							 "Game",			// the title bar text
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow )
{
	...
	//====================================================================
	// Load four textures for the Spaceships; yellow, green, red & explosions
	//====================================================================
	...
	// Initial starting position for Ships
	D3DXVECTOR3 balloonPos;

	gTheme.playSound(L"Sounds\\Theme.wav",false);

	/* initialize random seed: */
	srand ( (unsigned int)time(NULL) );

	/* generate random number of Ships */
	int numBalloons = (rand() % 12 + 1);
	//int numBalloons = (rand() % 10 + 1);
	...
	for(int loop = 0; loop < numBalloons; loop++)
	{
		...
		aBalloon[loop]->setTranslation(D3DXVECTOR2(5.0f,0.0f));
	...
	aSurface = d3dMgr->getD3DSurfaceFromFile("Images\\Cockpit.png");
	
	// load custom font
	cD3DXFont* balloonFont = new cD3DXFont(d3dMgr->getTheD3DDevice(),hInstance, "rmbunny");

	RECT textPos;
	//SetRect(&textPos, 50, 10, 550, 100);
	SetRect(&textPos, 200, 5, 750, 100);
	...
	while( msg.message!=WM_QUIT )
	{
		...
		else
		{
			...
			/*
			==============================================================
			| Update the postion of the Ships and check for collisions
			==============================================================
			*/
			if(timeElapsed > fpsRate)
			{
				...
				{
					(*iter)->update(timeElapsed);			// update Ship
					aballoonPos = (*iter)->getSpritePos();  // get the position of the current Ship
					/*
					==============================================================
					| Check for boundry collision and change ship direction
					==============================================================
					*/
